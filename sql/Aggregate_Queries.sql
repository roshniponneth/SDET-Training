...
select count(distinct salesman_id) as Salescount from orders

-- Activity 8 -perform aggregate operations using GROUP BY.
...
HAVING MAX(purchase_amount) IN(2030, 3450, 5760, 6000); --   Write an SQL statement to find the highest purchase amount with their ID and order date, for only those customers who have a higher purchase amount within the list [2030, 3450, 5760, 6000].
